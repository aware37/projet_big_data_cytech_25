"""Generate fare predictions from a pre-trained model.
...
.. code-block:: bash
    ...
    python scripts/predict.py \\
        --input s3://nyc-processed/cleaned/2022-02/ \\
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="Predict taxi fares with trained model"
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Cap rows (random sample).",
    )
    return p.parse_args()


def main():
    """Entry point: load model, predict, save CSV."""
    args = parse_args()
    paths = Paths()
